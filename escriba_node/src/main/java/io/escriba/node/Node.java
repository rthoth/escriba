...
public class Node {
	...
	public <T> Future<T> get(Postcard postcard, String key, int initialSize, PostcardReader0<T> reader) throws Exception {
		return postcard.get(this, key, initialSize, (total, entry, buffer) -> ReadAction.stop(reader.apply(entry, buffer)));
	}
	...
	public <T> Future<Postcard> put(Postcard postcard, String key, String mediaType, T content, PostcardWriter0<T> writer) throws Exception {
		return postcard.put(this, key, mediaType, null, content, (value, previous) -> WriteAction.stop(writer.apply(value)));
	}
