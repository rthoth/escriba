...
class Getter(TaskSet):
	...
	def get(self):
		...
		with self.client.delete('/%s/%d' % (col, key), catch_response=True) as response:
			if response.status_code == 200 and response.reason == '%s/%d' % (col, key):
				response.success()
			else:
				response.failure('%s/%d: %d - %s' % (col, key, response.status_code, response.reason))

		with self.client.get('/%s/%d' % (col, key), catch_response=True) as response:
